...
current cell's height. Water can flow from any cell
...
def pacific_atlantic(heights)
  pacific = Set.new
  atlantic = Set.new
  rows_length = heights.length
  cols_length = heights[0].length

  dfs = ->(row, col, visited, prev_height) do
    if (row < 0 ||
        col < 0 ||
        row == rows_length ||
        col == cols_length ||
    ...
    visited.add([row, col])
    dfs.call(row + 1, col, visited, heights[row][col])
    dfs.call(row - 1, col, visited, heights[row][col])
    dfs.call(row, col + 1, visited, heights[row][col])
    dfs.call(row, col - 1, visited, heights[row][col])
  end

  cols_length.times do |col|
    dfs.call(0, col, pacific, heights[0][col])
    dfs.call(rows_length - 1, col, atlantic, heights[rows_length - 1][col])
  end

  rows_length.times do |row|
    dfs.call(row, 0, pacific, heights[row][0])
    dfs.call(row, cols_length - 1, atlantic, heights[row][cols_length - 1])
  end

  result = []
  rows_length.times do |row|
    cols_length.times do |col|
      if (pacific.include?([row, col]) && atlantic.include?([row, col]))
        result << [row, col]
      end
    end
  end
