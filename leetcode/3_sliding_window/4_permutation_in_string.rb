=begin

https://leetcode.com/problems/permutation-in-string/description/

Given two strings s1 and s2, return true if s2 contains a permutation of s1, or false otherwise.
In other words, return true if one of s1's permutations is the substring of s2.

Example 1:

Input: s1 = "ab", s2 = "eidbaooo"
Output: true
Explanation: s2 contains one permutation of s1 ("ba").

Input: s1 = "ab", s2 = "eidboaoo"
Output: false

Constraints:
s1 and s2 consist of lowercase English letters.

=end


# @param {String} s1
# @param {String} s2
# @return {Boolean}

# USING HASH
def check_inclusion(s1, s2)
  return false if s2.length > 10**4
  return false if s1.size > s2.size
  left = 0
  right = s1.length-1
  s1_hash = Hash.new(0)
  s2_hash = Hash.new(0)


  s1.chars.each do |char|
    s1_hash[char] += 1
  end

  while right <= s2.length-1 do
    left_count = left

    # creatign s2 window hash
    # check for permutation by compare s2 hash with s1 hash
    res_c = 0
    while left_count <= right do
      s2_hash[s2[left_count]] += 1
      if s1_hash.key?(s2[left_count])
        if (s1_hash[s2[left_count]] == s2_hash[s2[left_count]])
          res_c += s1_hash[s2[left_count]]
        end
      end
      return true if res_c == s1.length
      left_count += 1
    end

    s2_hash.clear
    left += 1
    right += 1
  end
  false
end

p check_inclusion('ab', 'eidbaooo') # true
p check_inclusion('ab', 'eidboaoo') # false
p check_inclusion('adc', 'dcda') # true
p check_inclusion('hello', 'ooolleoooleh') # false
p check_inclusion('abcdxabcde', 'abcdeabcdx') # true

p check_inclusion(
  "",
  ""
) # true

p "-"*26

# HASH -> Leetcode Accepted
def check_inclusion(s1, s2)
  return false if s1.size > s2.size
  hash_1 = Hash.new(0)
  hash_2 = Hash.new(0)

  s1.chars.each_with_index do |char, index|
    hash_1[char] += 1
  end

  pointer_a = 0
  pointer_b = s1.length - 1

  s2.chars.each_with_index do |char, index|
    hash_2[char] += 1

    if index > s1.length-1
      if hash_2[s2[pointer_a]] > 1
        hash_2[s2[pointer_a]] -= 1
      else
        hash_2.delete s2[pointer_a]
      end

      pointer_a += 1
    end

    return true if hash_1 == hash_2
  end
  false
end

p check_inclusion('ab', 'eidbaooo') # true
p check_inclusion('ab', 'eidboaoo') # false
p check_inclusion('adc', 'dcda') # true
p check_inclusion('hello', 'ooolleoooleh') # false
p check_inclusion('abcdxabcde', 'abcdeabcdx') # true

p check_inclusion(
  "",
  ""
) # true

p "-"*26

def check_inclusion(s1, s2)
  return false if s1.size > s2.size
  s1_hash = Hash.new(0)
  s2_hash = Hash.new(0)
  counter = 0

  while counter < s1.length do
    s1_hash[s1[counter]] += 1
    s2_hash[s2[counter]] += 1

    counter += 1
  end

  while counter <= s2.length-1 do
    return true if s1_hash == s2_hash

    # remove old values
    s2_hash.clear

    # replace the pointer
    left = (counter - s1.length)+1
    right = counter

    # add new values
    while left <= right do
      s2_hash[s2[left]] += 1
      left += 1
    end

    counter += 1
  end

  s1_hash == s2_hash
end

p check_inclusion('ab', 'eidbaooo') # true
p check_inclusion('ab', 'eidboaoo') # false
p check_inclusion('adc', 'dcda') # true
p check_inclusion('hello', 'ooolleoooleh') # false
p check_inclusion('abcdxabcde', 'abcdeabcdx') # true

p check_inclusion(
  "",
  ""
) # true

p "-"*26

# SLIDING WINDOW - LEETCODE ACCEPTED  gt>1,gt>immediate_next
def check_inclusion(string, target)
  hash = Hash.new(0)
  checking = Hash.new(0)

  (0...string.length).each do |counter|
    hash[string[counter]] += 1
    checking[target[counter]] += 1
  end

  pointer_a = 0
  pointer_b = string.length-1

  while pointer_b < target.length do
    return true if hash == checking

    if checking[target[pointer_a]] > 1
      checking[target[pointer_a]] -= 1
    else
      checking.delete(target[pointer_a])
    end

    pointer_a += 1
    pointer_b += 1
    checking[target[pointer_b]] += 1
  end
   hash == checking
end

p check_inclusion('ab', 'eidbaooo') # true
p check_inclusion('ab', 'eidboaoo') # false
p check_inclusion('adc', 'dcda') # true
p check_inclusion('hello', 'ooolleoooleh') # false
p check_inclusion('abcdxabcde', 'abcdeabcdx') # true

p check_inclusion(
  "",
  ""
) # true


p "-"*26
# Efficient method - Sliding Window Technique
def check_inclusion(s1, s2)
  return false if s1.size > s2.size
  s1_array = [0] * 26
  s2_array = [0] * 26
  counter = 0

  while counter < s1.length do
    s1_array[s1[counter].ord - 'a'.ord] += 1
    s2_array[s2[counter].ord - 'a'.ord] += 1

    counter += 1
  end


  while counter < s2.length do
    return true if s1_array == s2_array

    left = s2[counter - s1.length]
    s2_array[left.ord - "a".ord] -= 1

    right = s2[counter]
    s2_array[right.ord - "a".ord] += 1

    counter += 1
  end

  s1_array == s2_array
end

p check_inclusion('ab', 'eidbaooo') # true
p check_inclusion('ab', 'eidboaoo') # false
p check_inclusion('adc', 'dcda') # true
p check_inclusion('hello', 'ooolleoooleh') # false
p check_inclusion('abcdxabcde', 'abcdeabcdx') # true

p check_inclusion(
  "",
  ""
) # true
p "-"*26
